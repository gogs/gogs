...
The Git LFS client communicates with the Gogs server over HTTP/HTTPS or SSH. Over HTTP, it uses HTTP Basic Authentication. Over SSH, it uses the same SSH key authentication as regular Git operations. When using SSH remotes with Git LFS 3.0+, LFS transfers happen natively over the SSH connection using the `git-lfs-transfer` protocol.
...
- [Git LFS client](https://git-lfs.github.com/) version **1.0.1** or later (version **3.0** or later for SSH transfer support)
...
<AccordionGroup>
  ...
  <Accordion title="SSH remotes require Git LFS 3.0+ for native transfers">
    Git LFS 3.0+ is required for native SSH transfers via the `git-lfs-transfer` protocol. Older Git LFS clients fall back to HTTP/HTTPS for LFS objects even when SSH is set as a remote.
  </Accordion>
