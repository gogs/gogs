...
<AccordionGroup>
  <Accordion title="What do I do if another service is already using port 3000?">
    ...
    Make sure to update `[server] HTTP_PORT` in `custom/conf/app.ini` to match, so subsequent restarts pick up the same port.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="How can I become an administrator?">
    ...
    Once logged in, the administrator can promote other users by navigating to **Admin Panel** > **Users** and editing user accounts.
  </Accordion>
