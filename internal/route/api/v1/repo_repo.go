...
func listUserRepositories(c *context.APIContext, username string) {
	...
	if c.User.ID != user.ID {
		...
		for i := range ownRepos {
			repos[i] = ToRepository(ownRepos[i], &types.Permission{Admin: true, Push: true, Pull: true})
		}
		c.JSONSuccess(&repos)
		return
	}
	...
	for _, r := range ownRepos {
		repos = append(repos, ToRepository(r, &types.Permission{Admin: true, Push: true, Pull: true}))
	}

	for repo, access := range accessibleReposWithAccessMode {
		repos = append(repos,
			ToRepository(repo, &types.Permission{
...
func CreateUserRepo(c *context.APIContext, owner *database.User, opt CreateRepoRequest) {
	...
	c.JSON(201, ToRepository(repo, &types.Permission{Admin: true, Push: true, Pull: true}))
}
...
func GetRepo(c *context.APIContext) {
	...
	c.JSONSuccess(ToRepository(repo, &types.Permission{
