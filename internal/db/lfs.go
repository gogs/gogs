...
// NewLFSStore returns a persistent interface for LFS objects with given
// database connection.
...
func (db *lfs) GetObjectByOID(ctx context.Context, repoID int64, oid lfsutil.OID) (*LFSObject, error) {
	...
	err := db.WithContext(ctx).
		Where("repo_id = ? AND oid = ?", repoID, oid).
		First(object).Error
	if err != nil {
...
func (db *lfs) GetObjectsByOIDs(ctx context.Context, repoID int64, oids ...lfsutil.OID) ([]*LFSObject, error) {
	...
	err := db.WithContext(ctx).
		Where("repo_id = ? AND oid IN (?)", repoID, oids).
		Find(&objects).Error
...
type LFSObject struct {
	RepoID    int64           `gorm:"primarykey;autoIncrement:false"`
