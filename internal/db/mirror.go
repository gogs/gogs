		...
		for _, result := range results {
			...
			if result.newCommitID == gitShortEmptyID {
				if err = Actions.MirrorSyncDelete(ctx, m.Repo, result.refName); err != nil {
					log.Error("MirrorSyncDelete [repo_id: %d]: %v", m.RepoID, err)
				}
				continue
			}
			...
			if result.oldCommitID == gitShortEmptyID {
				if err = Actions.MirrorSyncCreate(ctx, m.Repo, result.refName); err != nil {
					log.Error("MirrorSyncCreate [repo_id: %d]: %v", m.RepoID, err)
					continue
				}
				isNewRef = true
			}
			...
			err = Actions.MirrorSyncPush(ctx, m.Repo, result.refName, oldCommitID, newCommitID, CommitsToPushCommits(commits))
			if err != nil {
				log.Error("MirrorSyncPush [repo_id: %d]: %v", m.RepoID, err)
				continue
			}
		}
