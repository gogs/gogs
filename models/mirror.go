	...
	for repoID := range MirrorQueue.Queue() {
		...
		for _, result := range results {
			...
			if !isNewRef {
				...
				refUniqueCommitID, err := gitRepo.GetBranchCommitID(result.refName)
				if err != nil {
					log.Error(2, "GetFullCommitID [%d]: %v", m.RepoID, err)
					continue
				}
				if commit, err := gitRepo.GetCommit(refUniqueCommitID); err != nil {
					log.Error(2, "GetCommit [repo_id: %d, commit_id: %s]: %v", m.RepoID, refUniqueCommitID, err)
					continue
				} else {
					commits = list.New()
					commits.PushFront(commit)
					oldCommitID = refUniqueCommitID
					newCommitID = refUniqueCommitID
				}
			}
