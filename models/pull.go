...
func (pr *PullRequest) Merge(doer *User, baseGitRepo *git.Repository, mergeStyle MergeStyle, CommitMessage string) (err error) {
	defer func() {
	...
	case MERGE_STYLE_REGULAR: // Create merge commit
		...
		if _, stderr, err = process.ExecDir(-1, tmpBasePath,
			...
			"-m", strings.TrimSpace(fmt.Sprintf("Merge branch '%s' of %s/%s into %s\n%s", pr.HeadBranch, pr.HeadUserName, pr.HeadRepo.Name, pr.BaseBranch, CommitMessage))); err != nil {
