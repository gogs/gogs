...
func (repo *Repository) cloneLink(isWiki bool) *CloneLink {
	...
	if setting.SSH.Port != 22 {
		cl.SSH = fmt.Sprintf("ssh://%s@%s:%d/%s/%s.git", setting.SSH.User, setting.SSH.Domain, setting.SSH.Port, repo.Owner.Name, repoName)
	} else {
		cl.SSH = fmt.Sprintf("%s@%s:%s/%s.git", setting.SSH.User, setting.SSH.Domain, repo.Owner.Name, repoName)
	}
