...
- Sign-in sessions now persist across browser restarts by default for the duration of `[security] LOGIN_REMEMBER_DAYS`. The "Remember me" checkbox on the sign-in page is removed.
...
- The `[security] COOKIE_REMEMBER_NAME` and `[security] COOKIE_USERNAME` configuration options.
