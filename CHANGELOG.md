...
- The `gogs` binary is now on the `PATH` in the next-generation Docker image, so commands like `docker exec -it gogs gogs admin create-user` work without spelling out the full path to the binary.
