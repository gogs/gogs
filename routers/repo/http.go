...
func HTTP(ctx *context.Context) {
	...
	params[models.PROTECTED_BRANCH_USER_ID] = fmt.Sprintf("%d",authUser.ID)
	if (err == nil) {
		params[models.PROTECTED_BRANCH_ACCESS_MODE] = accessMode.String()
	}
	params[models.PROTECTED_BRANCH_REPO_ID] = fmt.Sprintf("%d",repo.ID)
...
func (h *serviceHandler) getBranch(input []byte) (string) {
	...
	for {
		...
		if head[0] == '0' && head[1] == '0' {
			...
			if len(fields) >= 3 {
				refFullName := fields[2]
				branchName = strings.TrimPrefix(refFullName, git.BRANCH_PREFIX)
			}
			lastLine = lastLine + size
		} else {
			break
...
func serviceRPC(h serviceHandler, service string) {
	...
	RepoID, _ := strconv.ParseInt(h.cfg.Params[models.PROTECTED_BRANCH_REPO_ID], 10, 64)
	accessMode := models.ParseAccessMode(h.cfg.Params[models.PROTECTED_BRANCH_ACCESS_MODE])
